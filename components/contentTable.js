    ...
    render(btnInnerText) {
        this.innerHTML = /* html */ `
            <div class="formAgg">
                ...
                <form-agregar action="" id='formulario'>
                    <div class="form first">
                        <div class="details personal">
                            <div class="fields">
                                <div class="input-field">
                                ...
                                <div class="input-field">
                                    ...
                                    <input type="text" placeholder="" required name="idTipoPersona">
                                </div>
                            </div>
                            <button>
                                <span class="btnText">${btnInnerText == 'Agregar' ? 'Crear': 'Guardar'}</span>
...
export class AgregarEstados extends HTMLElement{
    constructor(btnInnerText) {
        super();
        this.render(btnInnerText);
    }
...
export class AgregarTipoPersonas extends HTMLElement{
    constructor(btnInnerText) {
        super();
        this.render(btnInnerText);
    }
...
`,
    this.querySelector('#formulario').addEventListener('submit', async () => {
            let form = this.querySelector("#formulario");
            const data = Object.fromEntries(new FormData(form).entries());
            let lastId = await getLastId('tipoMovActivo'); // Reemplaza 'tipoMovActivo' con el nombre correcto de tu recurso
            data['id'] = ((lastId != (null || undefined) ? parseInt(lastId) : 0) + 1).toString();
            this.querySelector(".btnText") == 'Crear' ? await api.post(data,'tipoMovActivo') : await api.patch(data,'tipoMovActivo',this.idElement); // Reemplaza 'tipoMovActivo' con el nombre correcto de tu recurso
        });
    }
...
`,
    this.querySelector('#formulario').addEventListener('submit', async () => {
            let form = this.querySelector("#formulario");
            const data = Object.fromEntries(new FormData(form).entries());
            let lastId = await getLastId('proveedores'); // Reemplaza 'proveedores' con el nombre correcto de tu recurso
            data['id'] = ((lastId != (null || undefined) ? parseInt(lastId) : 0) + 1).toString();
            this.querySelector(".btnText") == 'Crear' ? await api.post(data,'proveedores') : await api.patch(data,'proveedores',this.idElement); // Reemplaza 'proveedores' con el nombre correcto de tu recurso
        });
    }
...
customElements.define('form-agregar',AgregarPersonas);
customElements.define('form-agregar-estados', AgregarEstados);
customElements.define('form-agregar-tipo-personas', AgregarTipoPersonas);
customElements.define('form-agregar-tipo-activo', AgregarTipoActivo);
customElements.define('form-agregar-tipo-mov-activo', AgregarTipoMovActivo);
customElements.define('form-agregar-proveedores', AgregarProveedores);
